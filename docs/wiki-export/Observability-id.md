> **English:** [Observability](Observability)
...
Semua di `/api/v1/metrics/traffic/*` (session required):
...
- `GET /dashboard` → `traffic_summary` dari `Summary(1h)`
- Traffic view Preact memanggil series/top-sites/status-codes endpoints
- Fallback: `GET /system/nginx-traffic` jika bucket belum terisi
