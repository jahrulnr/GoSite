> **Bahasa Indonesia:** [Website-nginx-config-id](Website-nginx-config-id)
...
Every `Reload()` calls `TestAndRepair` on the full production config before `nginx -s reload`. See [nginx-repair.md](Nginx-auto-repair).
