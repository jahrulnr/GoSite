> **Bahasa Indonesia:** [Container-startup-id](Container-startup-id)
...
`gosite nginx-repair` runs **after** the default SSL cert is created so fallback repair can point vhosts to the default cert. See [nginx-repair.md](Nginx-auto-repair).
