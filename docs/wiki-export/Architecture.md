> **Bahasa Indonesia:** [Architecture-id](Architecture-id)
...
Details: [sequences/01-container-startup.md](Container-startup)

`config/start.sh`:

1. `gosite init` — storage layout, symlinks, migrate, seed
2. Generate default self-signed SSL if missing
3. **`gosite nginx-repair`** — `nginx -t` + auto-fix ([nginx-repair.md](Nginx-auto-repair))
...
Certbot and website placeholder SSL share the `live/{domain}/` namespace. See [sequences/08-website-ssl.md](SSL-and-Certbot).
