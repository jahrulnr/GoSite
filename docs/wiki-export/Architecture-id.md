...
Detail: [sequences/01-container-startup.md](Container-startup-id))

`config/start.sh`:

1. `gosite init` — layout storage, symlink, migrate, seed
2. Generate default self-signed SSL jika belum ada
3. **`gosite nginx-repair`** — `nginx -t` + auto-fix ([nginx-repair.md](Nginx-auto-repair-id))
...
Certbot dan placeholder website create berbagi namespace `live/{domain}/`. Lihat [sequences/08-website-ssl.md](SSL-and-Certbot-id)).
