...
Session disimpan di SQLite (`sessions` table) dengan cookie HTTP-only. Lockscreen state in-memory (`auth.Lockscreen`).
...
- Lockscreen: `GET /locked` simpan user id di session
