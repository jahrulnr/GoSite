...
// Stable key per event. QueryEvent.id is a stable composite key (source|ts|message)
// generated by the backend; for robustness we fall back to content-derived key.
// With Live Tail new events are prepended, so positional keys would force Preact
// to remount every row. A stable content key keeps existing rows untouched.
function eventKey(event: QueryEvent): string {
  if (event.id) return event.id;
  const msg = event.message.length > 80 ? event.message.slice(0, 80) : event.message;
  return `${event.source}|${event.ts}|${event.action}|${msg}`;
}
...
const EventRow = memo(function EventRow({ event, isSmart, isOpen, onToggle }: EventRowProps) {
      ...
      <div class="evt-list" role="list">
        {events.map((event) => (
          <EventRow
            ...
            event={event}
            isSmart={isSmart}
            isOpen={expanded === eventKey(event)}
